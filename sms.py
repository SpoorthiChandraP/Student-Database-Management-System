...
def export_data():
    ...
    for index in indexing:
        ...
        datalist=content['values']  #row wise data in the form a list
        newlist.append(datalist)

    table=pandas.DataFrame(newlist,columns=['SRN','Name','Mobile No','Email','Address','Gender','DOB','Added Date','Added Time'])
    table.to_csv(url,index=False)
    messagebox.showinfo('Success','Data is saved successfully')


def toplevel_data(title,button_text,command):
    global srnEntry,nameEntry,phoneEntry,emailEntry,addressEntry,genderEntry,dobEntry,screen
    screen = Toplevel()
    screen.title(title)
    screen.grab_set()  # only after closing the window You click on some other button
    screen.resizable(0, 0)
    srnLabel = Label(screen, text='SRN', font=('times new roman', 20, 'bold'))
    srnLabel.grid(row=0, column=0, padx=30, pady=15, sticky=W)
    srnEntry = Entry(screen, font=('roman', 15, 'bold'), width=24)
    srnEntry.grid(row=0, column=1, pady=15, padx=10)
...
def update_data():
    query='update student set name=%s,mobile=%s,email=%s,address=%s,gender=%s,dob=%s,date=%s,time=%s where srn=%s'
    mycursor.execute(query,(nameEntry.get(),phoneEntry.get(),emailEntry.get(),addressEntry.get(),
                            genderEntry.get(),dobEntry.get(),date,currenttime,srnEntry.get()))
    con.commit()
    messagebox.showinfo('Success',f'SRN {srnEntry.get()} is updated successfully',parent=screen)
    screen.destroy()
    show_student()
...
def delete_student():
    ...
    query='delete from student where srn=%s'
    mycursor.execute(query,content_id)
    con.commit()
    messagebox.showinfo('Deleted',f'SRN {content_id} is deletd successfully')
    query='select * from student'
    mycursor.execute(query)
    fetched_data=mycursor.fetchall()
    studentTable.delete(*studentTable.get_children())
...
def search_data():
    query='select * from student where srn=%s or name=%s or mobile=%s or email=%s or address=%s or gender=%s or dob=%s'
    mycursor.execute(query,(srnEntry.get(),nameEntry.get(),phoneEntry.get(),emailEntry.get(),addressEntry.get(),genderEntry.get(),dobEntry.get()))
    studentTable.delete(*studentTable.get_children())
    fetched_data = mycursor.fetchall()
    for data in fetched_data:
        studentTable.insert('',END,values=data)
...
def add_data():
    if srnEntry.get() == '' or nameEntry.get() == '' or phoneEntry.get() == '' or emailEntry == '' or addressEntry.get() == '' or genderEntry.get == '' or dobEntry.get() == '':
        messagebox.showerror('Error','All fields are required.',parent=screen)

    else:
        try:
            query = 'insert into student values(%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            mycursor.execute(query,(srnEntry.get(),nameEntry.get(),phoneEntry.get(),emailEntry.get(),addressEntry.get(),
            ...
            messagebox.showerror('Error','SRN cannot be repeated',parent=screen)
            return
...
def connect_database():
    def connect():
        ...
        try:
            ...
            query = 'create table student(SRN int not null primary key, name varchar(30), mobile varchar(10), email varchar(30),' \
...
datetimeLabel = Label(root,text='hello',font=('times new roman',18,'bold'),foreground='dark red')
datetimeLabel.place(x=5,y=5)
clock()

s = 'Student Management System' #s[count]=S when count is 0     s[count]=t when count is 1
sliderLabel = Label(root,font=('arial',28,'italic bold'),width=30,foreground='dark red')
...
studentTable = ttk.Treeview(rightFrame,columns=('SRN','Name','Mobile No','Email','Address','Gender',
...
studentTable.heading('SRN',text='SRN')
...
studentTable.column('SRN',width=50,anchor=CENTER)
